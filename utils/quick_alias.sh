...
#
# Created alias is commented out so user can manually uncomment it
...
function bkdir() {
  ...
  if [ "$#" -eq 0 ]; then
    existsAlias="$(grep -c "alias $currDir" <  $CFG_DIR/.bash_quick_aliases)"

    if [ "$existsAlias" -eq 0 ]; then
      printf "%f" "\n No args passed, booking -> alias $currDir in .bash_aliases \n"
      echo "# alias $currDir='cd $(pwd)'" >>$CFG_DIR/.bash_quick_aliases
    else
      printf "\n else something \n"
    fi
  elif [ "$#" -eq 1 ]; then
    existsAlias="$(grep -c "alias $1" <  $CFG_DIR/.bash_quick_aliases)"

    if [ "$existsAlias" -eq 0 ]; then
      printf "%f" "\n Thank you for the arg, will add alias $1 in .bash_quick_aliases \n"
      echo "# alias $1='cd $(pwd)'" >>$CFG_DIR/.bash_quick_aliases
    else
      printf "\n else something \n"
    fi
  else
